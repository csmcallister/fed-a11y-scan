...
Note, however, that this step will not destroy the CloudFormation Stack, the S3 bucket created by `cdk bootstrap`, or the DynamoDB instance. There doesn't [seem](https://github.com/aws/aws-cdk/issues/986) to be a way to do this from the command line at present, so you should log into your AWS console and manually delete first the s3 bucket, then the Database, and then the CloudFormation Stack.
